...
public:
    ...
    void fixed(z3::expr const &ast, z3::expr const &value) override {
        ...
        if (assignment) {
            ...
            if (!addedInConflict) {
                ...
                isAlreadyOccupiedDiagonal2[diagonal2Idx] = diagonal2;
            }
        }
        else {
    ...
    ValidPlacement canBePlaced(z3::expr& ast) {
        std::pair<unsigned, unsigned> position = exprToPosition(ast);
        if ((z3::ast)isAlreadyOccupiedLine[position.second] ||
            (z3::ast)isAlreadyOccupiedColumn[position.first] ||
            (z3::ast)isAlreadyOccupiedDiagonal1[getDiagonal1Index(position.first, position.second)] ||
            (z3::ast)isAlreadyOccupiedDiagonal2[getDiagonal2Index(position.first, position.second)]) {

            return False;
        }
        if (assignedLine[position.second] == board || assignedColumn[position.first] == board) {
            return True;
        }
        return Undef;
    }
	...
	void decide(z3::expr& ast, unsigned&, Z3_lbool& phase) override {
		...
		switch (canBePlaced(ast)) {
            ...
            case False:
                ...
                return;
            case Undef:
                phase = rand() % 2 == 0 ? Z3_L_FALSE : Z3_L_TRUE;
                return;
		}
    }
};