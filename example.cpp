...
int test7(unsigned num) {
    ...
    WriteLine(solver.assertions().to_string());
    user_propagator_created_maximisation propagator(&solver, num);

    solver.check();
    z3::model model = solver.get_model();

    WriteLine("Model: " + model.to_string());
...
int main() {
    ...
    /*for (int num = MIN_BOARD; num <= MAX_BOARD; num++) {
    ...
    for (int num = MIN_BOARD; num <= MAX_BOARD; num++) {
        ...
        const char *testName[] =
                {
                        "BV + Quantifier + Blocking clauses",
                        "BV + SubQuery",
                        "BV + Assert Smaller",
                        "BV + Quantifier + Created",
                };
        int permutation[4] = {0, 1, 2, 3,};
        double timeResults[REPETITIONS * SIZE(permutation)];
